	...
	$.fn.TMSlider = $.fn.TMS = $.fn._TMS = function (o) {
		return this.each(function () {
			var th = $(this),
					...
					spiral: function spiral() {
						var opt = this,
							...
							for (i = step; i < w - 1 - step; i++) {
								if (ret.length < opt.maskC.length) ret.push(opt.matrix[step][i]);else return false;
							}rb();
						},
						    rb = function rb() {
							for (i = step; i < h - 1 - step; i++) {
								if (ret.length < opt.maskC.length) ret.push(opt.matrix[i][w - 1 - step]);else return false;
							}rl();
						},
						    rl = function rl() {
							for (i = step; i < w - 1 - step; i++) {
								if (ret.length < opt.maskC.length) ret.push(opt.matrix[h - 1 - step][w - i - 1]);else return false;
							}lt();
						},
						    lt = function lt() {
							for (i = step; i < h - 1 - step; i++) {
								if (ret.length < opt.maskC.length) ret.push(opt.matrix[h - i - 1][step]);else return false;
							}lr(step++);
						};
						lr();
						return ret;
					},
					vSnake: function vSnake() {
						var opt = this,
						    ret = [],
						    h = opt.blocksY,
						    w = opt.blocksX,
						...
						for (i = 0; i < w; i++) {
							for (j = 0; j < h; j++) {
								if (i * .5 == ~~(i / 2)) ret.push(opt.matrix[j][i]);else ret.push(opt.matrix[h - 1 - j][i]);
							}
						}return ret;
					},
					gSnake: function gSnake() {
						var opt = this,
						    ret = [],
						    h = opt.blocksY,
						    w = opt.blocksX,
						...
						for (i = 0; i < h; i++) {
							for (j = 0; j < w; j++) {
								if (i * .5 == ~~(i / 2)) ret.push(opt.matrix[i][j]);else ret.push(opt.matrix[i][w - 1 - j]);
							}
						}return ret;
					},
				...
				anims: {
					centralExpand: function centralExpand(el, last) {
						$(el).each(function () {
					...
					fadeThree: function fadeThree(el, last) {
						var _ = this;
						$(el).each(function (i) {
					...
					zoomer: function zoomer(el, last) {
						if (_.slideshow) _.slideshow = _.duration - 2000;
						...
						el.each(function () {
							var src = _.next,
							    ...
							    css,
							    animate = function animate(canvas, o) {
								var state,
								    im = {},
							    ...
							    calcCSS = function calcCSS(width, height, k) {
							...
							buff.css({
								...
								if (!ie) canvas.appendTo(wrap.appendTo(holder)).attr({
									width: holder.width(),
									height: holder.height()
								});else canvas = buff.css({
									position: 'absolute',
									left: 0,
									top: 0,
									zIndex: 1
					...
					expand: function expand(el, last) {
						var opt = this;
						$(el).each(function () {
					...
					slideDown: function slideDown(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this).show(),
							    h = th.height();
					...
					slideLeft: function slideLeft(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this).show(),
							    w = th.width();
					...
					slideUp: function slideUp(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this).show(),
							    h = th.height(),
					...
					slideRight: function slideRight(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this).show(),
							    w = th.width(),
					...
					slideFromTop: function slideFromTop(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this),
							    t = th.show().css('top'),
					...
					slideFromDown: function slideFromDown(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this),
							    t = th.show().css('top'),
					...
					slideFromLeft: function slideFromLeft(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this),
							    l = th.show().css('left'),
					...
					slideFromRight: function slideFromRight(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this),
							    l = th.show().css('left'),
					...
					gSlider: function gSlider(el, last) {
						var opt = this,
						...
						el.show().css({
							...
							easing: opt.easing,
							step: function step(now) {
								if (opt.direction > 0) clone.css('left', now + w);else clone.css('left', now - w);
							},
							complete: function complete() {
								clone.remove();
								if (last) opt.afterShow();
					...
					vSlider: function vSlider(el, last) {
						var opt = this,
						    clone = opt.maskC.clone(),
						    h = clone.height();
						clone.appendTo(opt.maskC.parent()).css({
						...
						el.show().css({
							...
							easing: opt.easing,
							step: function step(now) {
								if (opt.direction > 0) clone.css('top', now + h);else clone.css('top', now - h);
							},
							complete: function complete() {
								clone.remove();
								if (last) opt.afterShow();
					...
					vSlideOdd: function vSlideOdd(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this),
							    t = th.show().css('top'),
					...
					gSlideOdd: function gSlideOdd(el, last) {
						var opt = this;
						$(el).each(function () {
							var th = $(this),
							    l = th.show().css('left'),
				...
				preFu: function preFu() {
					...
					img.appendTo(_.me).load(function () {
						setTimeout(function () {
							...
							var t;
							if (_.preset == 'zoomer') t = _.n, _.n = -1, _.changeFu(t);
						}, 1);
					}).attr({ src: _.itms[_.n = _.show] });
				},
				sliceFu: function sliceFu(w, h) {
					var _ = this,
					    w = _.blocksX,
					    h = _.blocksY,
				...
				changeFu: function changeFu(n) {
					...
					var _fu = function _fu() {
						//if(_.banner)
						//$.when(_.banner).then(function(){_.banner.detach()})
						if ($.browser.msie && $.browser.version < 9 && _.preset == 'zoomer') _.preset = 'simpleFade', _.duration = 1000;
					...
					if (_.waitBannerAnimation) $.when(_.banner).then(_fu);else _fu();
				},
				...
				showFu: function showFu() {
					...
					_.int = setInterval(function () {
						if (way.length) _.anims[_.anim].apply(_, [way.shift(), !way.length]);else clearInterval(_.int); //,
						//$.when(_.maskC).then(function(){_.maskC.remove(),delete _.maskC})
					}, _.interval);
					_.bl = true;
				},
				dirMirrorFu: function dirMirrorFu(way) {
					var _ = this;
				...
				afterShow: function afterShow() {
					...
					if (_.banner) $.when($('.' + _.bannerCl, _.me)).then(function () {
				...
				paginationFu: function paginationFu() {
					...
					if (_.pagination === true) _.pags = $('<ul></ul>');else if (typeof _.pagination == 'string') _.pags = $(_.pagination);else if (_typeof(_.pagination) == 'object') _.pags = _.pagination.find('ul');
					if (_.pags.parent().length == 0) _.pags.appendTo(_.me);
					if (_.pags.children().length == 0) $(_.itms).each(function (n) {
						var li = $('<li></li>').data({ num: n });
						_.pags.append(li.append('<a href="#"></a>'));
					});else _.pags.find('li').each(function (n) {
						$(this).data({ num: n });
					});
					if (_.pagNums) _.pags.find('a').each(function (n) {
						$(this).text(n + 1);
					});
				...
				startSlShFu: function startSlShFu(prog) {
					var _ = this;
					_.paused = false;
					_.prog = prog || 0;
					clearInterval(_.slShTimer);
					_.slShTimer = setInterval(function () {
						if (_.prog < 100) _.prog++;else _.prog = 0, clearInterval(_.slShTimer), _.nextFu();
						if (_.progressBar) _.pbchFu();
					}, _.slideshow / 100);
					if (_.playBu) $(_.playBu).removeClass(_.pauseCl);
				},
				pauseSlShFu: function pauseSlShFu() {
					var _ = this;
					_.paused = true;
					clearInterval(_.slShTimer);
					if (_.playBu) $(_.playBu).addClass(_.pauseCl);
				},
				slideshowFu: function slideshowFu() {
					...
					if (_.playBu) $(_.playBu).bind(_.changeEv, function () {
						if (!_.paused) _.pauseSlShFu();else _.startSlShFu(_.prog);
						return false;
					});
					_.startSlShFu();
				},
				pbchFu: function pbchFu() {
					var _ = this;
					if (_.prog == 0) _.progressBar.stop().width(0);else _.progressBar.stop().animate({ width: _.prog / 100 * _.progressBar.parent().width() }, { easing: 'linear', duration: _.slideshow / 100 });
				},
				...
				bannersPresets: {
					fromLeft: {
						bannerShow: function bannerShow(banner, _) {
							if (banner.css('top') == 'auto') banner.css('top', 0);
							banner.stop().css({ left: -banner.width() }).animate({
								left: 0
							}, {
						...
						bannerHide: function bannerHide(banner, _) {
							banner.stop().animate({
								left: -banner.width()
							}, {
								duration: _.bannerDuration,
					...
					fromRight: {
						bannerShow: function bannerShow(banner, _) {
							...
							banner.stop().css({ right: -banner.width() }).animate({
								right: 0
							}, {
								duration: _.bannerDuration,
								easing: _.bannerEasing
					...
					fromBottom: {
						bannerShow: function bannerShow(banner, _) {
							if (banner.css('left') == 'auto') banner.css('left', 0);
							if (banner.css('top') != 'auto') banner.css('top', 'auto');
							banner.stop().css({ bottom: -banner.height() }).animate({
								bottom: 0
							}, {
								duration: _.bannerDuration,
								easing: _.bannerEasing
					...
					fromTop: {
						bannerShow: function bannerShow(banner, _) {
							if (banner.css('left') == 'auto') banner.css('left', 0);
							banner.stop().css({ top: -banner.height() }).animate({
								top: 0
							}, {
								duration: _.bannerDuration,
								easing: _.bannerEasing
				...
				numStatusFu: function numStatusFu() {
					...
					if (!_.numSt) if (_.numStatus === true) _.numSt = $(_.etal).addClass(_.numStatusCl);else _.numSt = $(_.numStatus).addClass(_.numStatusCl);
...
function swtch(arg, cases) {
	return typeof arg == 'string' || typeof arg == 'number' ? cases[arg] ? cases[arg] : cases['default'] || arg : (typeof arg === "undefined" ? "undefined" : _typeof(arg)) == 'object' ? function () {
		var ret = arg instanceof Array ? [] : {},
		    i;
		if (arg.constructor === RegExp) for (i in cases) {
			cases.hasOwnProperty(i) && arg.test(i) && (ret[i] = cases[i]);
		} else for (i in arg) {
			if (arg.hasOwnProperty(i)) ret[i] = swtch(arg[i], cases);
		}return ret;
